...
describe('The event sourced repository', () => {
  ...
  class CatRepository extends EventSourcedRepository {
    ...
    applyEvent(event, aggregateRoot) {
      if (event.type === 'catCreated') {
        Object.assign(aggregateRoot, event.payload);
      }
      if (event.type === 'catNamed') {
        Object.assign(aggregateRoot, {name: event.payload.name});
      }
    }
  }
  ...
  it('should have an aggregate type', () => {
    let act = () => new EventSourcedRepository().constructor.AggregateType;
