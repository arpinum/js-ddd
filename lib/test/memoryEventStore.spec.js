...
describe('The memory event store', () => {
  ...
  it('should add multiple events', () => {
    let addAll = eventStore.addAll([
      { first: 'first event' },
      { second: 'second event' }
    ]);

    return addAll.then(() => {
      eventStore.events.should.deep.equal([
        { id: 0, first: 'first event' },
        { id: 1, second: 'second event' }
      ]);
    });
  });
  ...
  it('should find events from a aggregate', () => {
    ...
    let eventsFromAggregate = eventStore.eventsFromAggregate(
      'rightAggregate',
      'rightType'
    );
    ...
    return arrayFromStream(eventsFromAggregate).then(results => {
      results.should.deep.equal([
        {
          id: 'event2',
          aggregate: { id: 'rightAggregate', type: 'rightType' }
        },
        { id: 'event4', aggregate: { id: 'rightAggregate', type: 'rightType' } }
      ]);
    });
