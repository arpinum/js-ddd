...
const {TypedRepository} = require('../types');
...
describe('The repository', () => {
  ...
  class Cat extends AggregateRoot {
    constructor(creation) {
      super({
        ...
        aggregateName: 'Cat',
        handlers: {
          CatCreated: (event, cat) => new Cat({id: cat.id, age: event.payload.age}),
          CatNamed: (event, cat) => new Cat({id: cat.id, age: cat.age, name: event.payload.name})
        }
      });
      this.age = creation.age;
  ...
  context('after creation', () => {
    it('should match TypedRepository', () => {
      ...
      TypedRepository.is(repository).should.be.true;
    });
  });
