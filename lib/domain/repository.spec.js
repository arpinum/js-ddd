...
describe('The repository', () => {
  ...
  context('while getting by id', () => {
    ...
    it('should use incoming event converters if provided before applying an event', () => {
      eventStore.events.push(
        new Event({
          id: '1',
          type: 'CatCreated',
  ...
  context('while saving events', () => {
    ...
    it('should use outcoming event converters if provided before saving an event', () => {
      ...
      return save.then(() => {
        eventStore.events.length.should.equal(1);
        eventStore.events[0].payload.birthDate.should.equal(date.toISOString());
      });
    });
