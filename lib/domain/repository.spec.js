  ...
  context('while getting by id', () => {
    ...
    it('should fail if event cannot be applied', () => {
      ...
      return getById.then(
        ...
        rejection => rejection.should.deep.equal(new TechnicalError('Event cannot be applied')));
    });
    ...
    it('should fail if root is missing', () => {
      ...
      return getById.then(
        () => Promise.resolve(new Error('Should fail')),
        rejection => rejection.should.deep.equal(new EntityNotFoundError('No entity for {"id":"missing_id"}')));
    });
