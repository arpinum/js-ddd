...
const Creation = t.interface({
  ...
  options: t.maybe(t.interface({
    ...
    beforeEventSave: t.maybe(t.list(t.Function))
  }))
}, {strict: true});
...
class Repository {
  ...
  getById(id, options) {
    ...
    function applyEvent(event, aggregateRoot, reject) {
      try {
        ...
        return _.flow(...self._options.beforeEventApplication,
          aggregateRoot.applyEvent.bind(aggregateRoot))(event);
      } catch (error) {
  ...
  saveEvents(events) {
    ...
    function storeEvents() {
      let decoratedEvents = events.map(applyDecorators);
      return self._eventStore.addAll(decoratedEvents);
    }

    function applyDecorators(event) {
      return _.flow(...self._options.beforeEventSave)(event);
    }
  }
}
