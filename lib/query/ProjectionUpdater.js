...
const QueueManager = require('../tools').QueueManager;
...
class ProjectionUpdater {
  ...
  _update() {
    ...
    return this._retrieveLastEventId()
      ...
      .then(() => {
        this._options.log.debug('Update done');
      });
  }
  ...
  _retrieveLastEventId() {
    if (!this._lastEventId) {
      return this._projectionStore.get(this.projectionName).then(result => {
        return _.get(result, 'lastEventId', null);
      });
    }
