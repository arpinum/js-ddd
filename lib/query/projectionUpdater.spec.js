...
describe('The projection updater', () => {
  ...
  context('while listening to event bus', () => {
    it('should handle relevant events', () => {
      let event = new Message({type: 'SomethingHappened', payload: {the: 'payload'}});
      updater.registerToBus(eventBus);

      let broadcast = eventBus.post(event);

      return broadcast.then(() => {
        handlings.should.deep.equal([{type: 'SomethingHappened', event}]);
      });
    });
    ...
    it('may handle event synchronically', () => {
      ...
      let broadcast = eventBus.post({type: 'SomethingHappened', payload: {the: 'payload'}});
    ...
    it('won\'t handle irrelevant events', () => {
      ...
      let broadcast = eventBus.post({type: 'IDoNotCare', payload: {the: 'payload'}});
